...
** Magit
   ...
   (defun fps/ediff-org-showhide (buf command &rest cmdargs)
     "If buffer exists and is orgmode then execute command"
     (when buf
       (when (eq (buffer-local-value 'major-mode (get-buffer buf)) 'org-mode)
         (save-excursion (set-buffer buf) (apply command cmdargs)))))

   (defun fps/ediff-org-unfold-tree-element ()
     "Unfold tree at diff location"
     (fps/ediff-org-showhide ediff-buffer-A 'org-reveal)  
     (fps/ediff-org-showhide ediff-buffer-B 'org-reveal)  
     (fps/ediff-org-showhide ediff-buffer-C 'org-reveal))

   (defun f-ediff-org-fold-tree ()
     "Fold tree back to top level"
     (fps/ediff-org-showhide ediff-buffer-A 'hide-sublevels 1)  
     (fps/ediff-org-showhide ediff-buffer-B 'hide-sublevels 1)  
     (fps/ediff-org-showhide ediff-buffer-C 'hide-sublevels 1))

   (add-hook 'ediff-select-hook 'fps/ediff-org-unfold-tree-element)
   (add-hook 'ediff-unselect-hook 'fps/ediff-org-fold-tree)
   #+END_SRC
